#!/usr/bin/env python3
"""
analyze_isolated_fairness.py
============================
Parse the output of run_isolated_fairness.py and report, per phase and per client,
the same metrics as analyze_shared_fairness.py -- adapted for the ISOLATED
topology (two independent top-level cgroups, no shared parent):
  ...
  iv)  memory consumption per client and total (sum of the two clients)
  v)   memory allocated: memory.max / memory.min of each client cgroup
  vi)  iostat device activity + per-client bandwidth (from fio)
  vii) speed of reading a page from memory (tmpfs) vs IO/disk (O_DIRECT)

What to look for vs the shared run: in ISOLATED mode client2 has its own capped
pool and cannot evict client1, so client1's phase-by-phase p99, refaults, and
memory.current should stay flat even while client2 bursts. That contrast with the
shared run is the fairness result.

Usage:  ./analyze_isolated_fairness.py isolated_results/
"""
import argparse
import json
import os
...
def fio_read(job_json):
    """Return read-side iops/bw/bytes/latency percentiles.
...
def main():
    ...
    print("=" * 92)
    print("ISOLATED FAIRNESS ANALYSIS")
    print(f"  client1 cgroup: {meta['cgroups']['client1']}  "
          f"(mem cap = {meta['mem_caps']['client1']})")
    ...
    # ---------------------------------------------------------------- v) allocation
    print("\n[v] MEMORY ALLOCATED (per isolated cgroup)")
    print(hr())
    s0 = load(f"{R}/phase0_stats.json")
    for c in ("client1", "client2"):
        snap = s0["before"][c]
        print(f"  {c:<8}({meta['cgroups'][c]:<12}) : "
              f"memory.max={snap.get('memory_max')}  memory.min={snap.get('memory_min')}")
    ...
    for i in range(nphases):
        ...
        # iv) total memory consumption (sum of the two isolated clients; no parent)
        print(f"\n  [iv] TOTAL (client1 + client2 memory.current) : {mib(total_cur):.0f} MiB")
